...
class HomeController extends Controller
{
    ...
    public function product_item(Request $request)
    {
        ...
        if (!$validator->fails()) {
            $product = ProductChild::where('id', $request->product_item_id )->first();
            if($product->count() == 0) {
                return new Response(['status'=>false, 'message' => 'This  product Not Found']);
            }
             $product["size"] = explode(',', $product->size);
    ...
    public function add_to_cart(Request $request)
    {
        ...
        if (!$validator->fails()) {
            ...
            $data = Carts::where('user_id',$user->id)->where('product_id',$request->product_item_id)->where('size',$request->size)->count();
            $cart = Carts::where('user_id',$user->id)->where('product_id',$request->product_item_id)->where('size',$request->size)->first();
            if($data == 1) {
    ...
    public function add_location(Request $request)
    {
        ...
        if (!$validator->fails()) {
            ...
            ->select('carts.*', 'items.id as product_id','items.name','items.Description','items.price','items.image','items.color')
            ->where('user_id', $user->id )
            ->get();
    ...
    public function add_fav(Request $request)
    {
       ...
       if (!$validator->fails()) {
          ...
          $is_fav =Favorit::where('user_id',$user->id)
          ->where('product_id',$request->product_id)->count();
        if($is_fav != 0) {
            return [
                'status' => false,
                'message' => 'this product found in Favorite' ,
            ];
        }
    ...
    {
        $user = $request->user('web-api');
        $points = Points::where('user_id',$user->id)->orderBy('created_at', 'desc')->paginate(7);
    ...
    {
        $validator = Validator($request->all(), [
            'filter_category' => ['exists:categories,id'],
            'filter' => ['nullable'],
        ]);
        ...
        if (!$validator->fails()) {
            ...
            $products =Product::where('category_id', $request->filter_category )
            ->get();
