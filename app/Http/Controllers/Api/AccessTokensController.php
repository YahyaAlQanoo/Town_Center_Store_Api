...
class AccessTokensController extends Controller
{
    ...
    public function forgotPassword(Request $request)
    {
        ...
        if (! $validator->fails()) {
            ...
            if (is_null($user->reset_code)) {
                ...
                $user->reset_code = Hash::make($resetCode);
                $saved = $user->save();
                Mail::to($user)->send(new AdminResetPasswordEmail($user,$resetCode));
                ...
                return new Response(['ststus'=>false, 'message' => 'Reset Password code sent ']);
            }
         }else {
            return new Response(['ststus'=>false, 'message'=>$validator->getMessageBag()->first()], Response::HTTP_BAD_REQUEST);
        }
    ...
    public function resetPassword(Request $request) {
        $validator = Validator($request->all(),[
            ...
            'new_password' => ['required',Password::min(3)->letters()->symbols()->mixedCase()->uncompromised()], 
        ]);
        ...
        if (! $validator->fails()) {
            ...
            if (! is_null($user->reset_code)) {
                if (Hash::check($request->reset_code, $user->reset_code)) {
                    ...
                    $saved = $user->save();
                    return new Response(['ststus'=>$saved, 'message'=>$saved ? 'Password Reset successfully ' : 'Password Reset   Failed'], $saved ? Response::HTTP_CREATED : Response::HTTP_BAD_REQUEST);
                }
     
            }else {
                return new Response(['ststus'=>false, 'message' => 'Password Reset   Failed ']);
            }
        }else {
            return new Response(['ststus'=>false, 'message'=>$validator->getMessageBag()->first()], Response::HTTP_BAD_REQUEST);
        }
